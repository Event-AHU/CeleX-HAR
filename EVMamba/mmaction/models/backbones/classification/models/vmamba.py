...
from .patchembed_event import PatchEmbed_event
...
class VSSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.patchembed_event = PatchEmbed_event()
    ...
    def forward(self, x: torch.Tensor, z: torch.Tensor):  # torch.Size([bs*8, 3, 224, 224]), # torch.Size([bs*1, 131, 4096])
        x = self.patch_embed(x)      # torch.Size([24, 56, 56, 128])
        z = self.patchembed_event(z) # torch.Size([3, 56, 56, 128])
        for layer in self.layers:  
            x = layer(x)
            z = layer(z)
        z = z.repeat(8,1,1,1)
        x = x + z
        x = self.classifier(x)  # torch.Size([24, 1000])
        return x
