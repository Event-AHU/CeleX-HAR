...
class ActionDataPreprocessor(BaseDataPreprocessor):
    ...
    def forward_onesample(self, data, training: bool = False) -> dict:
        ...
        """
        if 'imgs' in data:
            inputs, data_samples = data['imgs'], data['data_samples']
            inputs, data_samples = self.preprocess(inputs, data_samples, training)
            data['inputs'] = inputs
            data['data_samples'] = data_samples
        if 'event_voxel' in data:
            event_voxel = data['event_voxel']
            
            import torch.nn.functional as F
            for i in range(len(event_voxel)):
                event_voxel[i] = event_voxel[i].unsqueeze(0).unsqueeze(0)
                if event_voxel[i].shape[2] < 4096:
                    pad_len = 4096 - event_voxel[i].shape[2]
                else:
                    # event_voxel, _ = torch.topk(event_voxel, k=4096, dim=2)
                    event_voxel[i] = event_voxel[i][:,:,:4096,:]
                    pad_len = 0
                event_voxel[i] = F.pad(event_voxel[i].transpose(-1, -2), (0, pad_len), mode='constant', value=0)
                event_voxel[i] = event_voxel[i].squeeze(0).squeeze(0).transpose(1,0)
            event_voxel = torch.stack(event_voxel, dim=0)

            mean = torch.mean(event_voxel)
            std = torch.std(event_voxel)
            normalized_event_voxel = (event_voxel - mean) / std
            data['inputs_voxel'] = normalized_event_voxel
