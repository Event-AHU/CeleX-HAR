...
# from .hr_preprocessor import Preprocessor
import scipy.io as scio
from .trajectories import Tracks
...
class RawFrameDecode(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        # rearrange the voxel grids
        voxel_file_path = results['voxel_dir']
        voxel_mat_path = os.path.join(voxel_file_path, directory.split(os.sep)[-2], directory.split(os.sep)[-1]) + ".mat"
        
        if os.path.exists(voxel_mat_path):
            if os.path.getsize(voxel_mat_path) == 0:
                event_features = torch.zeros(4096, 131)
                # need_data = [np.zeros([4096, 3]), np.zeros([4096, 128])]  # the number of voxel grid = 4096
            else:
                try:
                    mat_data = scio.loadmat(voxel_mat_path)
                except Exception as e:
                    print(f"An unexpected error occurred while reading the file at {voxel_mat_path}. {str(e)}")
                # need_data = [mat_data['coor'], mat_data['features']]
                event_features = Tracks()(mat_data['coor'], mat_data['features'], min_length=1)   # coorelate and features (t,x,y feauture)
                # event_features = np.concatenate((mat_data['coor'], mat_data['features']), axis=1)
                if torch.isnan(event_features).any():
                    event_features = torch.zeros(4096, 131)
                    print(voxel_mat_path, 'exist nan value in voxel.')
        else:
            raise FileNotFoundError("miss", voxel_mat_path)
      
        results['imgs'] = imgs
        results['event_voxel'] = event_features
        results['original_shape'] = imgs[0].shape[:2]
        results['img_shape'] = imgs[0].shape[:2]
